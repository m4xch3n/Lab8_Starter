describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodButton = await root.$('button');
    await prodButton.click();
    const innerText = await prodButton.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < 20; i++) {
      const shadowRoot = await prodItem[i].getProperty('shadowRoot');
      const prodButton = await shadowRoot.$('button');
      await prodButton.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItem = await page.$$('product-item');
    const count = await page.$('#cart-count');
    let countStaysSame = true;
    for(let i = 0; i < 20; i++) {
        const shadowRoot = await prodItem[i].getProperty('shadowRoot');
        const productButton = await shadowRoot.$('button');
        const buttonText = await productButton.getProperty('innerText');
        if(await buttonText.jsonValue() != "Remove from Cart") {
          countStaysSame = false;
        }
    }
    const cartText = await count.getProperty('innerText');
    if(await cartText.jsonValue() != 20) {
      countStaysSame = false;
    }
    expect(countStaysSame).toBe(true);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < 20; i++) {
      const shadowRoot = await prodItem[i].getProperty('shadowRoot');
      const prodButton = await shadowRoot.$('button');
      const buttonText = await prodButton.getProperty('innerText');
      if(await buttonText.jsonValue() != "Add to Cart") {
        countStaysSame = false;
      }
    }
    const cartText = await count.getProperty('innerText');
    if(await cartText.jsonValue() != 0) {
      countStaysSame = false;
    }
    expect(countStaysSame).toBe(true);
  }, 10000);
